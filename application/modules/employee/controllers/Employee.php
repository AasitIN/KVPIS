...
if (!defined('BASEPATH'))
    ...
    public function pushMail($Sub,$Msg,$To){
        ...
        $AuthMail=$this->config->item('smtp_user');

        
        
        $this->email->set_newline("\r\n");
        $this->email->from($AuthMail,"KVS");
        $this->email->reply_to($AuthMail,'KVS');
        $this->email->to($To);
        ...
        if ($emp_per_data = $this->input->post(null, true)) { 
            ...
            if ($this->form_validation->run($this) !== FALSE) {
                ...
                if (($response['status'] == 'success')&&($response['action'] == 'INS')) {
                    ...
                    $Msg='Hi, Your registration successfully done.';
                    $To='asita.kumar@uneecops.com';
                    $this->pushMail($Sub,$Msg,$To);
